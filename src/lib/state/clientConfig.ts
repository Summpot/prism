...
export const EMPTY_CLIENT_CONFIG: ClientConfigResponse = {
	...
	active_config: {
		...
		optimizer_buffer_threshold: 65536,
	},
	profiles: [],
	cumulative_stats: {
		raw_bytes: 0,
...
function persistPayload(data: ClientConfigResponse) {
	return {
		...
		active_config: {
			...
			optimizer_enabled: data.active_config.optimizer_enabled,
			optimizer_zstd_level: data.active_config.optimizer_zstd_level,
			optimizer_adaptive_flush: data.active_config.optimizer_adaptive_flush,
			optimizer_flush_interval_ms: data.active_config.optimizer_flush_interval_ms,
			optimizer_buffer_threshold: data.active_config.optimizer_buffer_threshold,
