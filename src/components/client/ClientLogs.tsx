...
export function ClientLogs() {
	...
	return (
		<div className="mx-auto flex h-full w-full max-w-5xl flex-1 min-h-0 flex-col gap-2 p-3 sm:p-4 overflow-hidden">
			<PageHeader
				icon={<Terminal className="h-4 w-4 text-primary" />}
				title={m.client_logs_title()}
				badge={
					<Badge variant="outline" className="text-[10px] px-1.5 py-0 h-4 font-mono flex-none">
						{filteredLogs.length} / {logs.length}
					</Badge>
				}
				description={m.client_logs_description()}
				actions={
